...
params = set('byr iyr eyr hgt hcl ecl pid'.split())
passports = []
count = 0
...
for p in passports:
    chunk = p.split()
    actual = set([c.split(':')[0] for c in chunk])
    if len(params - actual) == 0:
        valids += 1
        d = dict([c.split(':') for c in chunk])
        c1 = len(d['byr']) == 4 and 1920 <= int(d['byr']) <= 2002
        c2 = len(d["iyr"]) == 4 and 2010 <= int(d["iyr"]) <= 2020
        c3 = len(d["eyr"]) == 4 and 2020 <= int(d["eyr"]) <= 2030
        c4 = (re.match("^\d\d\dcm$", d["hgt"]) is not None and 150 <= int(d["hgt"][:-2]) <= 193) or (
                    re.match("^\d\din$", d["hgt"]) is not None and 59 <= int(d["hgt"][:-2]) <= 76)
